...
async function confirmSession(sessionId, userId, options = {}) {
  ...
  const dogDoc = {
    ...
    vaccinations: calendar.vaccines
      .filter(item => item.vaccineType && item.vaccineType.trim()) // Only include records with vaccineName
      .map((item) => ({
    ...
    dewormingHistory: calendar.deworming
      .filter(item => item.productName && item.productName.trim()) // Only include records with productName
      .map((item) => ({
