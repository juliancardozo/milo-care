...
async function computeMetrics(partner, month, { now = new Date() } = {}) {
  ...
  for (const user of users) {
    for (const dog of user.dogs || []) {
      ...
      totalPets += 1;
      cohortDogIds.push(dog._id);
      if (isPetActive(dog, month)) activePets += 1;
      if (isPetActive(dog, prevMonth)) activePrevMonth += 1;
      if (dogUpToDate(dog, now)) upToDate += 1;
      if (dog.sponsorshipStatus === 'sponsored') sponsoredPets += 1;
      countEventsInMonth(dog, month, eventsByType);
    }
  }

  // Mascotas con verificación veterinaria activa (atestación vigente). Útil para la
  // aseguradora; sigue siendo un agregado (no expone qué se atestó).
  const verifiedDogIds = cohortDogIds.length
    ? await VetAttestation.distinct('dogId', { dogId: { $in: cohortDogIds }, status: 'active' })
    : [];
  ...
  return {
    ...
    partner: { name: partner.name, slug: partner.slug, type: partner.type, branding: partner.branding || {} },
    month,
    totalPets,
    activePets,
    activePrevMonth,
    sponsoredPets,
    verifiedPets: verifiedDogIds.length,
